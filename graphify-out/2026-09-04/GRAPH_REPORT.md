# Graph Report - gpq  (2026-09-04)

## Corpus Check
- 120 files · ~141,319 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2959 nodes · 7535 edges · 96 communities (84 shown, 9 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 188 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `00cf5fcd`
...
2. `ContentHash` - 96 edges
3. `BackendError` - 47 edges
4. `Harness` - 42 edges
5. `ArtifactRow` - 37 edges
6. `ArtifactManifest` - 35 edges
7. `ApiError` - 35 edges
8. `FailureKind` - 32 edges
...
Cohesion: 0.05
Nodes (94): build_fixtures(), fake(), Fixtures, harness(), interrupt_cancels_running_image_generation(), object_store(), object_store_output_placement_lands_in_bucket_and_redirects(), openai_image_generation_returns_sdk_compatible_base64_json() (+86 more)

### Community 1 - "gpq-remote/src/artifacts.rs"
Cohesion: 0.06
Nodes (72): Bytes, ArtifactService, as_u64(), checked_grow(), ChunkDrainOutcome, classify_download(), consume_worker_local_output(), deliver_request() (+64 more)
...
Cohesion: 0.08
Nodes (53): check_device_overlap(), duration_secs(), ensure_state_dir(), parses_full_sample(), parses_mlx_dspark_pool(), PoolConfig, rejects_backend_url_query_and_fragment(), rejects_duplicate_pool_keys() (+45 more)
...
Cohesion: 0.09
Nodes (37): accept_lease(), backoff_base(), cancel_all_attempts(), cancel_attempt(), cancel_expired_leases(), classify_lease(), deliver_artifact(), discard_output() (+29 more)
...
Cohesion: 0.09
Nodes (27): Backend, Send, Sync, acquire_fails_on_an_unready_pool(), acquire_never_exceeds_capacity(), exit_detection_marks_pool_unready_and_clears_capacity(), lock_state(), mark_unready_for_exit() (+19 more)

### Community 15 - "executor.rs"
Cohesion: 0.05
Nodes (71): ArtifactManifest, manifest(), MediaKind, String, verification_accepts_matching_bytes(), verification_rejects_short_transfer(), verification_rejects_wrong_digest(), FailureKind (+63 more)
...
Cohesion: 0.06
Nodes (39): apply_override(), apply_parameters(), apply_parameters_overrides_addressed_node_input(), apply_parameters_places_seed_at_reserved_pointer(), apply_parameters_rejects_malformed_pointer(), apply_parameters_rejects_missing_portable_placeholder(), apply_parameters_rejects_unknown_node_id(), apply_parameters_replaces_portable_placeholders() (+31 more)

### Community 20 - "native/catalog.rs"
Cohesion: 0.08
Nodes (40): CatalogService, catalog_error(), CatalogApi, custom_node_without_exact_version_is_rejected(), internal(), invalid(), missing_mime_type_is_rejected(), missing_output_node_is_rejected() (+32 more)
...
Cohesion: 0.13
Nodes (51): ContentHash, Display, CatalogError, content_hash_changes_with_graph(), content_hash_ignores_limits(), content_hash_is_stable_under_key_reordering(), decode_error(), decode_model_execution_limits() (+43 more)

### Community 39 - "SlotCapability"
Cohesion: 0.13
Nodes (29): BTreeSet, accelerator_memory_boundary_is_inclusive(), any_candidate_remains(), extra_models_and_custom_nodes_do_not_block_admission(), hash(), IncapableReason, missing_custom_node_reports_no_installed_version(), mlx_dspark_admits_llm_model_requirements() (+21 more)

### Community 41 - "scheduler.rs"
Cohesion: 0.12
Nodes (23): build_lease_assignment(), duration_from_micros_round_trips(), duration_from_negative_micros_falls_back_to_zero(), lease_target_fields(), leased_output_key(), model_target_leaves_workflow_fields_unset(), proto_artifact_manifest(), proto_artifact_placement() (+15 more)
...
Cohesion: 0.08
Nodes (17): data_url_decodes_base64(), data_url_decodes_percent_encoded_text(), ErrorBody, ErrorDetail, ipv6_mapped_private_is_rejected(), is_publicly_routable(), is_publicly_routable_v4(), is_publicly_routable_v6() (+9 more)
...
Cohesion: 0.13
Nodes (26): ComfyBackend, custom_node_versions(), custom_node_versions_falls_back_to_unknown(), custom_node_versions_prefers_known_package_version(), default_view_kind(), history_output(), internal_error(), ObjectInfoEntry (+18 more)
...
Cohesion: 0.16
Nodes (15): build_admission_request(), data_url_enforces_size_cap(), InputArtifactGuard, media_kind_from_mime(), resolve_and_store_images(), ArtifactId, Drop, IntoIterator (+7 more)
...
Cohesion: 0.21
Nodes (15): ApiError, await_terminal_generation(), failure_status_and_code(), fetch_generation_row(), GenerationId, GenerationRow, Into, Receiver (+7 more)

### Community 62 - "hash.rs"
Cohesion: 0.12
Nodes (12): digest_matches_known_vector(), round_trips_through_hex(), D, Err, Error, Formatter, FromStr, Result (+4 more)

### Community 63 - "Modality"
Cohesion: 0.16
Nodes (17): Modality, ExecutionLimits, ModelVersion, resolve_execution_timeout(), BTreeMap, Duration, MediaKind, Option (+9 more)
...
### Community 67 - "PoolSupervisor"
Cohesion: 0.26
Nodes (10): PoolSupervisor, Receiver, Sender, connect_and_serve(), Arc, CancellationToken, Client, Result (+2 more)

### Community 68 - "PoolAdvertisementData"
Cohesion: 0.22
Nodes (9): PoolAdvertisementData, BTreeMap, String, Vec, spawn_stream_logger(), unready_reason(), pool_advertisement(), PoolAdvertisement (+1 more)
...
### Community 88 - ".invalid_request"
Cohesion: 0.29
Nodes (12): fetch_http_image(), ip_literal_host_public_address_is_accepted(), parse_data_url(), resolve_image_input(), resolve_safe_addrs(), ResolvedImage, Duration, Result (+4 more)
...
### Community 90 - "wrap_worker_message"
Cohesion: 0.36
Nodes (8): capability_report_message(), deliver_artifact_message(), heartbeat_message(), DeliverArtifactRequest, Into, Message, WorkerMessage, wrap_worker_message()

### Community 91 - "main.ts"
Cohesion: 0.29
Nodes (5): gpq, prompt, REFINE_INSTRUCTIONS, size, { values, positionals }
...
Cohesion: 0.33
Nodes (5): AbortOnDrop, MaintenanceTask, Drop, JoinHandle, Option

## Knowledge Gaps
- **83 isolated node(s):** `UsageJson`, `DeltaEvent`, `REFINE_INSTRUCTIONS`, `{ values, positionals }`, `size` (+78 more)
...
- **Why does `AppState` connect `AppState` to `gpq-remote/src/artifacts.rs`, `native/generation.rs`, `tenants.rs`, `chat.rs`, `GenerationEvent`, `native/catalog.rs`, `responses.rs`, `transfer.rs`, `gpq-remote/src/session.rs`, `native/mod.rs`, `src/cli.rs`, `admission.rs`, `record_accepted_result`, `images.rs`, `scheduler.rs`, `Db`, `openai/mod.rs`, `gpq-remote/src/config.rs`, `plan_assignments`, `.new`, `.enroll`, `resolve_and_store_images`, `sse.rs`, `ApiError`, `run_inbound_pump`, `list_models`, `http.rs`, `recv_or_pending`, `SchedulerHandle`?**
  _High betweenness centrality (0.250) - this node is a cross-community bridge._
- **Why does `ContentHash` connect `ContentHash` to `workers.rs`, `generations.rs`, `llama.rs`, `gpq-worker/src/session.rs`, `pool.rs`, `executor.rs`, `native/catalog.rs`, `transfer.rs`, `mlx.rs`, `gpq-worker/src/artifacts.rs`, `SlotCapability`, `backend/mod.rs`, `BackendError`, `plan_assignments`, `src/models.rs`, `schedule.rs`, `Candidate`, `hash.rs`, `Modality`, `BackendKind`, `PoolAdvertisementData`, `list_models`, `ExecutionTarget`?**
...
- **What connects `UsageJson`, `DeltaEvent`, `REFINE_INSTRUCTIONS` to the rest of the system?**
